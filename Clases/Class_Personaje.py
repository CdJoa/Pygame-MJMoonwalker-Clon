...
class Personaje:
    def __init__(self, animaciones,animaciones2,tamaño, pos_x, pos_y,velocidad) -> None:
        ...
        if not os.path.isfile("datos_jugador.json"):
            # Supongamos que tienes los datos del jugador en un diccionario llamado datos_jugador
            self.salud = 20
            self.vidas =2
        else:
            ...
            # Asignar los datos del jugador a las variables correspondientes
            self.salud = datos_jugador.get("salud", 100)
            self.vidas = datos_jugador.get("vida", 3)
            self.Puntaje = datos_jugador.get("puntaje", 0)
        ...
        self.asombro_sound = py.mixer.Sound("Recursos/Aaow!.mp3")
        self.golpeado_sound = py.mixer.Sound("Recursos/Oh!.mp3")
        self.disparo_sound = py.mixer.Sound("Recursos/laser.wav")
        self.muerto_sound=py.mixer.Sound("Recursos\muerteMJ.wav") 
        self.muerte_enemigo =py.mixer.Sound("Recursos\muerte_enemigos.wav")
        self.muerte_mj_sound=py.mixer.Sound( "Recursos\muerteMJ.wav") 

        self.lista_sonidos = [self.asombro_sound,self.golpeado_sound,self.disparo_sound,self.muerto_sound,self.muerte_enemigo,self.muerte_mj_sound]
    ...
    def guardar_datos_jugador(self):
        # Guardar los datos del jugador en datos_jugador.json
        datos_jugador = {"salud": self.salud, "vida": self.vidas, "puntaje": self.Puntaje, "nivel completado": self.nivelCompletado, "nivel actual": self.nivel_actual,"muerto":self.muerto, "Muertes": self.contador_Muertes,"Tiempo": self.Tiempo,"Puntaje Nivel":self.PuntajeNivel}
    ...
    def manejar_efectos_sonido(self):
        with open("datos_volumen.json", "r") as nivel2_file:
            ...
            self.volumen_efectos = content.get("volumen_efectos", 0)
