...
class EntreNiveles(Form):
    ...
    def btn_home_click(self, parametro):
        ...
        with open("datos_niveles.json", "r+") as json2_file:
            datos_niveles = json.load(json2_file)

            if datos_niveles.get("medio", 0) == 1:
                datos_niveles["medio"] = 0
        
            json2_file.seek(0)   
            json.dump(datos_niveles, json2_file, indent=2)
            json2_file.truncate()
    ...
    # Draw the background image on the main surface
        ...
        with open("datos_jugador.json", "r") as json_file:
            ...
            puntaje = str(datos_jugador["puntaje"])
            muertes_texto = f"Puntaje Inicial: {puntaje}            x 1"
            text_surface1 = font.render(muertes_texto, True, ("white"))  # Assuming white text color
            self._slave.blit(text_surface1, (W/8, H/16)) 

            

            vidas = str(datos_jugador["vida"])
            vidas_texto = f" Vidas: {vidas}                 x 100 " 
            text_surface1 = font.render(vidas_texto, True, ("white"))  # Assuming white text color
            self._slave.blit(text_surface1, (W/8, H/8)) 

            
            tiempo = str(datos_jugador["Tiempo"])
            tiempo_texto = f"+    Tiempo: {tiempo}                 x 1"
            text_surface1 = font.render(tiempo_texto, True, ("white"))  # Assuming white text color
            self._slave.blit(text_surface1, (W/10, H/5)) 

            muertes = str(datos_jugador["Muertes"])
            muertes_texto = f"Muertes: {muertes}                 x- 100"
            text_surface1 = font.render(muertes_texto, True, ("white"))  # Assuming white text color
            self._slave.blit(text_surface1, (W/8, H/3)) 

            
            salud = str(datos_jugador["salud"])
            salud_texto = f"Salud: {salud}%                 x 10  "
            text_surface = font.render(salud_texto, True, ("white"))  # Assuming white text color
            self._slave.blit(text_surface, (W/8, H/2.5))

            PuntajeFinal = str(datos_jugador["Puntaje Nivel"])
            PuntajeFinal_texto = f"Puntaje Final     {PuntajeFinal}"
            text_surface = font.render(PuntajeFinal_texto, True, ("white"))  # Assuming white text color
